...
function UISkillCoreDetailView:InitPowerUpView(data, addExp)
  ...
  if 1 > self.mImage_AftFill.transform.localScale.x then
  ...
  if nextLvExp <= sumExp and self.mData.level < self.mStcData.max_level then
  ...
  if addExp <= 0 or curCost > playerCash or self.mData.level >= self.mCoreMaxLevel then
