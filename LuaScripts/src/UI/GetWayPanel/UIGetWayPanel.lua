...
function UIGetWayPanel:ctor(csPanel)
  UIGetWayPanel.super.ctor(self, csPanel)
...
function UIGetWayPanel:OnInit(root, data)
  self = UIGetWayPanel
...
function UIGetWayPanel:GetItemGetType(getList)
  ...
  if typeList[9] ~= nil and getWayStoryUpperLimit < #typeList[9] then
    table.sort(typeList[9], function(a, b)
      local tValueA, tValueB
      tValueA, tValueB = self:CheckIsUnLock(a.jump_code) == true or false, self:CheckIsUnLock(b.jump_code) == true or false
