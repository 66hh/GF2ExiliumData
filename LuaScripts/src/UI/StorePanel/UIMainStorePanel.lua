...
UIMainStorePanel.E3DModelType = gfenum({
  "eUnkown",
  "eGun",
  "eWeapon",
  "eEffect",
...
function UIMainStorePanel:InitAdvertisement()
  ...
  self.mPageView = self.mView.mTrans_AdvertisementList.transform.parent:GetComponent(typeof(CS.PageView))
...
function UIMainStorePanel.ShowStorePanel(tag, isQuickPurchase)
  ...
  UIManager.OpenUIByParam(UIDef.UIStorePanel, params)
end
function UIMainStorePanel.OnSkinClicked(gameObj)
  self = UIMainStorePanel
  self.mGelinaData = NetCmdNpcData:GetNpcDataById(self.mStcGelinaId)
  UIManager.OpenUIByParam(UIDef.UIStoreSkinPanel, {
    self.mGelinaData,
    2
  })
...
function UIMainStorePanel.UpdateAffectionBar(npcData)
  ...
  CS.ProgressBarAnimationHelper.Play(self.mView.mImage_AffectionBar_AffectionFilledBar, start_level, end_level, duration, function(curLv)
    print(curLv)
...
function UIMainStorePanel:CreateModel(modelId)
  ...
  if not self.mSelectedGunId:Contains(modelId) then
    ...
    local animator = self.mGunModel.gameObject:GetComponent(typeof(CS.UnityEngine.Animator))
...
function UIMainStorePanel:SetLookAtCharacter(obj)
  ...
  self.mCharacterSelfShadowSettings = obj:GetComponent(typeof(CS.Logic.Ligthing.CharacterSelfShadowSettings))
