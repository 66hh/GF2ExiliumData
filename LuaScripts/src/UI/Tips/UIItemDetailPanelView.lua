...
function UIItemDetailPanelView:ShowGet(cmdCallBack)
  ...
  for i = 1, #t do
    local tableID = t[i]
    if NetCmdStageRecordData:CheckHasReqDataByType(tableID:GetHashCode()) == false and cmdCallBack == nil then
...
function UIItemDetailPanelView:ShowStamina()
  ...
  if self.mCount and 0 <= tonumber(self.mCount) then
    self.mText_Count.text = tostring(self.mCount)
  else
    self.mText_Count.text = ""
  end
  if not self.mNeedGetWay and not self.mShowTime then
    ...
    setactive(self.mText_AllAdd.transform.parent, maxStamina > self.mCount)
    setactive(self.mText_NextAdd.transform.parent, maxStamina > self.mCount)
    setactive(self.mText_Full.gameObject, maxStamina <= self.mCount)
    self.updateFlag = maxStamina > self.mCount
  end
end
function UIItemDetailPanelView:UpdateStaminaContent()
  ...
  if staminaInfo then
    ...
    local lastTime = 0
    if maxStaminaInScene > self.mCount then
