...
function UIFriendBlackItem:InitInfo()
  if self.avatarInfo == nil then
    ...
    UIUtils.GetButtonListener(self.avatarInfo.ui.mBtn_Avatar.gameObject).onClick = function()
...
function UIFriendBlackItem:SetData(data)
  ...
  self.mText_Level.text = GlobalConfig.SetLvText(self.playerData.Level)
end
...
function UIFriendBlackItem:OnClickPlayerInfo()
  if self.playerData then
    UIManager.OpenUIByParam(UIDef.UIPlayerInfoPanel, self.playerData)
  end
end
