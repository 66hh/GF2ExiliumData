...
function UIBarrackChrCardItem:__InitCtrl()
  ...
  self.mTrans_SelectBlack = self:GetRectTransform("Trans_GrpChoose")
...
function UIBarrackChrCardItem:SetData(gunId, needRedPoint, ItemId, hasTip)
  if gunId then
    ...
    if self.cmdData then
      ...
      self.mImage_AvatarImage.color = Color(0.403921568627451, 0.44313725490196076, 0.45098039215686275, 0.9647058823529412)
...
function UIBarrackChrCardItem:UpdateRedPoint()
  ...
  if self.cmdData then
    ...
    count = count + NetCmdTeamData:UpdateWeaponModRedPoint(self.cmdData)
  else
