...
UIChapterInfoPanel.DifficultyType = {Normal = 1, Hard = 2}
  ...
  if data then
    ...
    for i = 0, data.Count - 1 do
      ...
      if item then
        ...
        elseif UIChapterInfoPanel.curChapterId == data[i].id then
          curChapter = item
        end
      end
    end
    if 0 < data.Count then
      ...
      setactive(self.mTrans_Hard_Unlock, UIChapterInfoPanel.curDiff == UIChapterInfoPanel.DifficultyType.Normal and not self:CheckHardModeIsUnLock(data[0]))
    end
    self.needUpdate = true
...
function UIChapterInfoPanel:OnClickDiffChange(mode)
  ...
  if mode == UIChapterInfoPanel.DifficultyType.Hard and not self.IsHardModeUnLocked then
