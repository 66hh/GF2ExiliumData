...
function UIHardChapterItem:SetData(data, isHard, index)
  if data ~= nil then
    ...
    self.isHard = isHard
    self.isNext = self.isUnLock == 0 and self.levelUnlocked and 0 < NetCmdDungeonData:UpdateChapterRedPoint(data.id)
...
function UIHardChapterItem:UpdateRedPoint()
  ...
  setactive(self.mTrans_RedPoint, self.isUnLock == 0 and self.levelUnlocked and 0 < NetCmdDungeonData:UpdateChatperRewardRedPoint(self.data.id))
